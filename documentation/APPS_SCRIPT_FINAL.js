...
function completeDeploymentNow_(scheduleFullName, scheduleId, botId, deploymentId, token) {
  ...
  // Update bot status to idle in BOTH Firestore and RTDB
  if (botId) {
 ...
 * @param {string|null} deploymentId - Current deployment ID or null
...
function summarizeTelemetry_(botId, deploymentId, token) {
  ...
  // Prefer time-series readings under deployments/{id}/readings
  ...
  // Fallback to bot snapshot under bots/{botId}
  if (botId) {
