 ...
 * UPDATED VERSION - Now updates bot status in BOTH Firestore AND RTDB
 *
...
function processScheduledToActive_() {
  ...
  scheduled.forEach(row => {
    try {
      ...
      const deploymentId = getFsString_(fields, CFG.FIELDS.schedule.deploymentId) || scheduleId;

      // Ensure deployment doc exists
      ensureDeployment_(deploymentId, {
      ...
      patchDoc_(doc.name, {
        ...
        [CFG.FIELDS.schedule.deploymentId]: { stringValue: deploymentId }
      ...
      console.log(`Activated schedule ${scheduleId} (deployment ${deploymentId})`);

      // Optional immediate completion if already past endAt
      if (endAt && endAt <= now) {
        completeDeploymentNow_(doc.name, scheduleId, botId, deploymentId, token);
      }
    } catch (e) {
...
/**
 * Completes a deployment + schedule, aggregates telemetry, and resets bot.
 */
function completeDeploymentNow_(scheduleFullName, scheduleId, botId, deploymentId, token) {
  const nowIso = new Date().toISOString();

  // Aggregate telemetry
  const metrics = summarizeTelemetry_(botId, deploymentId, token);

  // Update deployment
  const depFullName = `projects/${CFG.PROJECT_ID}/databases/(default)/documents/${CFG.FS.DEPLOYMENTS}/${deploymentId}`;
  ...
  // Reset bot to idle in BOTH Firestore and RTDB
  if (botId) {
    updateBotStatusBoth_(botId, 'idle', null, null, token);
  }

  console.log(`Completed schedule ${scheduleId} / deployment ${deploymentId}`);
}

/* =================== Bot status update (FIRESTORE + RTDB) =================== */

/**
 * Update bot status in BOTH Firestore and Realtime Database.
 * This ensures bot cards update in real-time in the Flutter app.
 */
...
function summarizeTelemetry_(botId, deploymentId, token) {
  const out = {
    totalTrashKg: 0,
    avgPH: null,
    avgTurbidity: null,
  ...
  const depReadingsPath = `/${CFG.RT.DEPLOYMENTS_ROOT}/${deploymentId}/readings`;
  const readings = rtdbGet_(depReadingsPath, token);
  if (readings && typeof readings === 'object') {
    out.source = depReadingsPath;
    accumulateFromReadings_(readings, out);
    return out;
  }
